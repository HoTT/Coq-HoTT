...
Proof.
  refine (equiv_compose' _ (equiv_sigma_symm (fun b c => f b = g c))).
  refine (equiv_functor_sigma' (equiv_idmap C) _); intros c.
...
Section Functor_Pullback.
  ...
  Proof.
    ...
    refine (equiv_compose' _ (hfiber_functor_sigma _ _ _ _ _ _)).
    refine (equiv_functor_sigma' (equiv_idmap _) _).
    intros [b1 e1]; simpl.
    refine (equiv_compose' _
              (equiv_transport (fun x => hfiber (functor_sigma l _) x) _ _
                               (transport_sigma' e1^ (c2; e2)))).
    refine (equiv_compose' _
              (hfiber_functor_sigma _ _ _ _ _ _)); simpl.
    refine (equiv_functor_sigma' (equiv_idmap _) _).
    intros [c1 e3]; simpl.
    refine (equiv_compose' _
    ...
    refine (equiv_functor_sigma' (equiv_idmap _) _); intros e0; simpl.
    refine (equiv_compose' (equiv_concat_l (transport_paths_Fl e0 _) _) _).
    refine (equiv_compose'
              (equiv_concat_l (whiskerL (ap h e0)^ (transport_paths_r e2 _)) _) _).
    refine (equiv_compose' (equiv_moveR_Vp _ _ _) _).
    refine (equiv_compose' (equiv_concat_l (concat_pp_p _ _ _) _) _).
    refine (equiv_compose' (equiv_moveR_Vp _ _ _) _).
    do 2 refine (equiv_compose' (equiv_concat_r (concat_pp_p _ _ _) _) _).
