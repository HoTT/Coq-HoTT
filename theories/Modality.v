Require Import Overture PathGroupoids types.Empty types.Unit types.Arrow types.Sigma types.Forall types.Prod HProp Equivalences UnivalenceAxiom.
...
Section Reflexive_Subuniverse.
  ...
  Section Basic_facts.
    ...
    Definition unique_subuniverse : forall (T T' : subuniverse_Type subU), T.1 = T'.1 -> T = T'.
    ...
    Definition O_unit_retract_equiv (T:Type) (μ : (subU.(O) T).1 -> T) (η := subU.(O_unit) T) : Sect η μ -> IsEquiv η.
    ...
    Definition subuniverse_iff_O (T:Type) :
      ...
      exact (@equiv_iff_hprop
               ...
               (fun X => subuniverse_struct_transport _ _ (BuildEquiv _ _ _ (isequiv_inverse (H:=X))) ((subU.(O) T)).2)
    ...
    Definition O_rec_O_unit (A : subuniverse_Type subU) (B : Type) (f : B -> A.1) (x : (O subU B).1) : O_unit subU A.1 (O_rec B A f x) = O_rec B (O subU A.1) ((O_unit subU A.1) o f) x
      :=  ((inverse ((fun U => (ap10 (O_rec_sect B (O subU A.1) U) x)) (O_unit subU A .1 o O_rec B A f))) @ (inverse (ap (fun u => O_rec B (O subU A.1) (O_unit subU A.1 o u) x) (inverse (O_rec_retr B A f)))))%path.

    (** The universal property commutes with eta *)

    Definition equal_fun_modal (A:Type) (B:subuniverse_Type subU) (f g:(O subU A).1 -> B.1) (η := O_unit subU A) : ((f o η = g o η) -> (f=g))
      := fun H => ((concat (concat (inverse (eissect _ (IsEquiv := (O_equiv subU A B)) f)) (ap equiv_inv H)) (eissect _ (IsEquiv := (O_equiv subU A B)) g))).

    Lemma universality_unit_lemma (oA A B: Type) (η : A -> oA) (f g : oA -> B) (inv : (A -> B) -> oA -> B) (π : f o η = g o η) (eisretr : forall x:A -> B, (inv x) o η = x) (eissect : forall x : oA -> B, inv (x o η) = x) a : ap10 (ap inv π) (η a) = ((ap10 (eisretr (f o η)) a @ ap10 π a) @ (ap10 (eisretr (g o η)) a) ^)%path.
      destruct π. simpl. rewrite concat_p1. rewrite concat_pV. exact idpath.
    Qed.

    Definition universality_unit (A:Type) (B:subuniverse_Type subU) (f g:(O subU A).1 -> B.1) (η := O_unit subU A) (π : (f o η = g o η)) : forall a, ap10 (equal_fun_modal A B _ _ π) (η a) = ap10 π a.
  ...
  Section Functions_lifts.
    ...
    Definition function_lift (A B : Type) (f : A -> B) : (subU.(O) A).1 -> (subU.(O) B).1.
      apply O_rec; intro x; apply subU.(O_unit); apply f; exact x.
    Defined.

    Definition function_lift_modal (A:Type) (B:subuniverse_Type subU) (f : A -> B.1) : (O subU A).1 -> B.1.
      apply O_rec. exact f.
    Defined.
    ...
    Definition function_lift_modal_square (A : Type) (B : subuniverse_Type subU) (f : A -> B.1) : (@equiv_inv _ _ (subU.(O_unit) B.1) (O_modal_equiv _ B)) o (function_lift A B.1 f) o (subU.(O_unit) A) =  f.
      apply path_forall; intro x; unfold compose, function_lift; simpl.
      exact (transport (fun U => O_rec B .1 B (fun x : (B) .1 => x) U = f x) (inverse (ap10 ((O_rec_retr A (subU.(O) B.1)) ((O_unit subU B.1) o f)) x)) (ap10 (O_rec_retr B.1 B idmap) (f x))).
    Defined.
    ...
    Definition function_lift_compose (A B C : Type) ( f : A -> B) (g : B -> C) :
      ...
      assert (P1 : O_rec A (O subU C) (fun x0 : A => O_unit subU C ((g o f) x0)) x = O_rec A (O subU C) (((O_unit subU C) o g) o f) x).
      exact idpath.

      assert (P2 : O_rec A (O subU C) (((O_unit subU C) o g) o f) x = O_rec A (O subU C) (((O_rec B (O subU C) (O_unit subU C o g) o O_unit subU B) o f)) x).
      apply ap10. apply ap. apply ap10. apply ap.
      exact (inverse (O_rec_retr B (O subU C) (O_unit subU C o g))).

      assert (P3 : O_rec A (O subU C) (O_rec B (O subU C) (O_unit subU C o g) o O_unit subU B o f) x = O_rec A (O subU C) (O_rec B (O subU C) (O_unit subU C o g) o (O_unit subU B o f)) x).
      exact idpath.

      assert (P4 : O_rec A (O subU C)
                         (O_rec B (O subU C) (O_unit subU C o g) o (O_unit subU B o f)) x =
                   O_rec A (O subU C)
                         (O_rec B (O subU C) (O_unit subU C o g) o (O_rec A (O subU B) (O_unit subU B o f) o O_unit subU A)) x).
      apply ap10. repeat apply ap.
      exact (inverse (O_rec_retr A (O subU B) (O_unit subU B o f))).
      exact (P1 @ P2 @ P3 @ P4 @ (ap10 (O_rec_sect A (O subU C) (O_rec B (O subU C) (O_unit subU C o g) o O_rec A (O subU B) (O_unit subU B o f))) x))%path.
    Defined.

    (** Hence function lift is ok with commutative squares *)

    Definition function_lift_square (A B C X : Type) (π1 : X -> A) (π2 : X -> B) (f : A -> C) (g : B -> C) (comm : (f o π1) = (g o π2)) : ( (function_lift A C f) o (function_lift X A π1) ) = ( (function_lift B C g) o (function_lift X B π2) ).
      unfold function_lift, compose in *; simpl in *.
      apply path_forall; intro x; simpl.

      pose (foo1 := ap10 (function_lift_compose X A C π1 f) x). unfold function_lift, compose in foo1; simpl in foo1.
      pose (foo2 := ap10 (function_lift_compose X B C π2 g) x). unfold function_lift, compose in foo2; simpl in foo2.
  ...
  Section Types.
    ...
    Definition forall_subuniverse (A:Type) (B:A -> Type) : (** Theorem 7.7.2 *)
      (forall x, (subU.(subuniverse_HProp) (B x)).1) -> ((subU.(subuniverse_HProp)) (forall x:A, (B x))).1.
      intro H.
      ...
      pose (foo := @O_rec_retr subU (forall x0 : A, (B x0)) (B x; H x) (fun f : forall x0 : A, (B x0) => f x)).
      exact (ap10 foo φ).
    Qed.
    ...
    Lemma product_universal (A B : Type) (C : subuniverse_Type subU) :
      ...
      exists (fun f : (((O subU A) ) .1 -> (existT (fun S => (subuniverse_HProp subU S).1) (B  -> (C ) .1) (arrow_subuniverse B  C)).1) =>
                f o O_unit subU A).
    ...
    (** Dependent sums *)
    Definition sigma_subuniverse : (** Theorem 7.7.4 *)
      (forall (A:subuniverse_Type subU) (B:A.1 -> subuniverse_Type subU),
         (subuniverse_HProp subU ({x:A.1 & (B x).1})).1) <->
      (forall (A:Type) (B: (O subU A).1 -> subuniverse_Type subU) (g : forall (a:A), (B (O_unit subU A a)).1), {f : forall (z:(O subU A).1), (B z).1 & forall a:A, f (O_unit subU A a) = g a}).
      split.
      - intro H. intros A B g.
        pose (Z := existT (fun T => (subuniverse_HProp subU T).1) ({z:(O subU A).1 & (B z).1}) (H (O subU A) B)).
        pose (g' :=( fun a:A => (O_unit subU A a ; g a)) : A -> Z.1).
        pose (f' := O_rec _ _ g').
        pose (eqf :=fun a:A => (ap10 (O_rec_retr _ _ g') a)). unfold compose in eqf. fold f' in eqf.
        pose (g'' := fun x => (f' x).1).
        pose (f'' := fun x:(O subU A).1 => x).
        pose (eq'' := path_forall _ _ (fun x => @ap _ _ pr1 _ _ (eqf x))).
        assert (g'' o (O_unit subU A) = f'' o (O_unit subU A)).
        exact eq''.
        pose (eq''' := ap10 (equal_fun_modal _ A (O subU A) (g'') (f'') (eq''))). unfold f'', g'' in eq'''; simpl in eq'''.
      ...
      - intros H A B.
        ...
        pose (k := (fun w => (h w; f w)) : (O subU ({x:A.1 & (B x).1})).1 -> ({x:A.1 & (B x).1})); simpl in k.
        ...
        apply (path_sigma _ (O_rec {x : A .1 | (B x) .1} A pr1
                             ...
                             f (O_unit subU {x : A .1 | (B x) .1} (x1; x2))) (x1;x2) (p (x1;x2))).
    ...
    Lemma pullback_subuniverse (A B C : subuniverse_Type subU) (f : A.1 -> C.1) (g : B.1 -> C.1) : (subU.(subuniverse_HProp) (pullback f g)).1.
      rewrite <- subuniverse_iff_O.
      (* TODO *)
    Admitted.

    Lemma paths_subuniverse (S:subuniverse_Type subU) (x y:S.1) : (subuniverse_HProp subU (x=y)).1.
