...
Section AssumeStuff.
  ...
  Definition Graph@{v u} := @sig@{u u} Type@{v} (fun V => @sig@{u u} (V -> V -> Type@{v}) (fun E => forall x y, IsHProp (E x y))).

  (** We also write out its constructors and fields explicitly to control their universes. *)
  Definition Build_Graph@{v u} (vert : Type@{v}) (edge : vert -> vert -> Type@{v})
             (ishprop_edge : forall x y, IsHProp (edge x y))
    : Graph@{v u}
    := @exist@{u u} Type@{v} (fun V => @sig@{u u} (V -> V -> Type@{v}) (fun E => forall x y, IsHProp (E x y)))
             vert
             (@exist@{u u} (vert -> vert -> Type@{v}) (fun E => forall x y, IsHProp (E x y))
                    edge ishprop_edge).
  Definition vert@{v u} : Graph@{v u} -> Type@{v} := pr1.
  Definition edge@{v u} (A : Graph@{v u}) : vert A -> vert A -> Type@{v} := pr1 (pr2 A).
  Instance ishprop_edge@{v u} (A : Graph@{v u}) (x y : vert A) : IsHProp (edge A x y)
    := pr2 (pr2 A) x y.
  (** We will need universe annotations in a few more places, but not many. *)

  Definition equiv_path_graph0 (A B : Graph@{v u})
    : { f : vert A <~> vert B &
  ...
  Definition equiv_path_graph@{v u} := equiv_path_graph0@{v u u u u}.

  (** N will be the set of graphs generated by the empty graph as
  "zero", and "adding a new top element" as "successor". *)
  Definition graph_zero@{v u} : Graph@{v u}
    := Build_Graph Empty
                   (fun x y => @Empty_rec@{u} Type@{v} x)
                   (fun x y => Empty_rec x).

  Definition graph_succ@{v u} (A : Graph@{v u}) : Graph@{v u}.
  Proof.
    srefine (Build_Graph@{v u} (sum@{v v} (vert A) Unit) _ _).
    - intros [x|x] [y|y].
      + exact (edge A x y).
      + exact Unit.
      + exact Empty.
  ...
  Definition graph_succ_top@{v u} {A : Graph@{v u}} (x : vert (graph_succ A))
    : edge (graph_succ A) x (inr tt).
  Proof.
    destruct x as [x|x]; exact tt.
  Qed.

  Definition graph_succ_top_unique@{v u}
             {A : Graph@{v u}} (y : vert (graph_succ A))
  ...
  Definition graph_succ_not_top@{v u} {A : Graph@{v u}} (x : vert A)
    : ~(edge (graph_succ A) (inr tt) (inl x))
    := idmap.

  Definition graph_succ_not_top_unique@{v u} {A : Graph@{v u}} (x : vert (graph_succ A))
  ...
  Section Graph_Succ_Equiv.
    Universes v u.
    Context {A B : Graph@{v u}} (f : vert (graph_succ A) <~> vert (graph_succ B))
    ...
    Proof.
      destruct x.
      srefine (transport@{v Set} is_inr graph_succ_equiv_inr^ tt).
    Qed.

    Definition graph_unsucc_equiv_vert@{} : vert A <~> vert B
      := equiv_unfunctor_sum_l@{v v v v v  v Set Set Set Set}
                              f Ha Hb.
  ...
  Defined.
  (* Coq bug: doing Def foo@{v u} ... above complains about unbound universes. *)
  Definition graph_succ_path_equiv@{v u}
    := Eval unfold graph_succ_path_equiv0 in graph_succ_path_equiv0@{v u}.

  Definition graph_unsucc_path@{v u} (A B : Graph@{v u})
  ...
  [graph_succ]. *)
  Definition in_N@{v u p} (n : Graph@{v u})
    := forall (P : Graph@{v u} -> Type@{p}),
           (forall A, IsHProp (P A))
           -> P graph_zero@{v u}
           -> (forall A, P A -> P (graph_succ A))
           -> P n.

  Lemma ishprop_in_N0 (n : Graph) : IsHProp (in_N@{v u p} n).
  Proof.
    apply trunc_forall.
  Qed.
