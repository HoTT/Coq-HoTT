...
Section AssumeStuff.
  ...
  Definition Build_Graph@{v e hp u} (vert : Type@{v}) (edge : vert -> vert -> Type@{e})
    ...
    : Graph@{v e hp u}
    := @exist@{u u} Type@{v} (fun V => @sig@{u u} (V -> V -> Type@{e}) (fun E => forall x y, IsHProp@{hp} (E x y)))
             vert
             (@exist@{u u} (vert -> vert -> Type@{e}) (fun E => forall x y, IsHProp@{hp} (E x y))
                    edge ishprop_edge).
  Definition vert@{v e hp u} : Graph@{v e hp u} -> Type@{v} := pr1.
  ...
  Definition graph_zero@{v e hp u} : Graph@{v e hp u}
    := Build_Graph@{v e hp u}
                  Empty
                  (fun x y => @Empty_rec@{u} Type@{e} x)
                  (fun x y => @Empty_rec@{hp} (IsHProp@{hp} (@Empty_rec@{u} Type@{e} x)) x).

  Definition graph_succ@{v e hp u} (A : Graph@{v e hp u}) : Graph@{v e hp u}.
  Proof.
    srefine (Build_Graph@{v e hp u} (sum@{v v} (vert A) Unit) _ _).
    - intros [x|x] [y|y].
      + exact (edge A x y).
      + exact Unit.
      + exact Empty.
  ...
  [graph_succ]. *)
  Definition in_N@{v e hp u p} (n : Graph@{v e hp u})
    := forall (P : Graph@{v e hp u} -> Type@{p}),
           (forall A, IsHProp (P A))
           -> P graph_zero@{v e hp u}
