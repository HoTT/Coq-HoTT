...
Section cauchy.
  ...
  Section sequence.
    ...
    Section modulus_close.
      ...
      Context {M} `{CauchyModulus M}.

      Lemma modulus_close_limit : forall {l}
                                         (islim : IsLimit l)
                                         (epsilon : Qpos Q),
      ...
      Proof.
        intros ???.
        assert (lim_close := is_limit (epsilon / 2));
          strip_truncations.
        destruct lim_close as [N isclose'].
        set (n := Peano.max (M (epsilon / 2)) N).
        ...
        assert (leMM : M (epsilon / 2) ≤ M (epsilon / 2) ) by apply (Peano.le_n).
        assert (x_close := cauchy_convergence (epsilon/2) n (M (epsilon / 2)) leMn leMM).
