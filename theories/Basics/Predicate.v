...
Global Instance reflexive_pred_eq {A : Type} : Reflexive (@pred_eq A)
  := reflexive_pointwise (fun _ => _).

Global Instance symmetric_pred_eq {A : Type} : Symmetric (@pred_eq A)
  := symmetric_pointwise (fun _ => _).

Global Instance transitive_pred_eq {A : Type} : Transitive (@pred_eq A)
...
(** TODO: move *)
Global Instance reflexive_fun : Reflexive (fun A B => A -> B)
  := fun _ => idmap.

(** TODO: move *)
Global Instance transitive_fun : Transitive (fun A B => A -> B)
  := fun _ _ _ f g => g o f.

(** The subset relation is reflexive. *)
Global Instance reflexive_pred_subset {A : Type} : Reflexive (@pred_subset A)
  := reflexive_pointwise (B:=fun x => Type) (fun _ A B => A -> B).

(** The subset relation is transitive. *)
Global Instance transitive_pred_subset {A : Type} : Transitive (@pred_subset A)
