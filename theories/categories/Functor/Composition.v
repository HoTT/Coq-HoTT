...
Section FunctorComposition.
  ...
  Let compose_functors_composition_of' s d d'
  ...
  Proof.
    ...
    reflexivity.
  Defined.
  Definition compose_functors_composition_of s d d' m1 m2
    := Eval cbv beta iota zeta delta
            [compose_functors_composition_of' internal_paths_rew] in
        @compose_functors_composition_of' s d d' m1 m2.

  Let compose_functors_identity_of' x
  : c_morphism_of (identity x) = identity (c_object_of x).
  Proof.
    rewrite <- !identity_of.
    reflexivity.
  Defined.
  Definition compose_functors_identity_of x
    := Eval cbv beta iota zeta delta
            [compose_functors_identity_of' internal_paths_rew] in
        @compose_functors_identity_of' x.

  Global Arguments compose_functors_composition_of / .
  Global Arguments compose_functors_identity_of / .
  Global Opaque compose_functors_composition_of compose_functors_identity_of.

  Definition compose_functors : Functor C E
    := Build_Functor
         C E
         (fun c => G (F c))
         (fun _ _ m => morphism_of G (morphism_of F m))
         compose_functors_composition_of
         compose_functors_identity_of.
End FunctorComposition.

Infix "o" := compose_functors : functor_scope.
