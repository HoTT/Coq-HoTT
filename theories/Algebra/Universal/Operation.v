...
Monomorphic Definition head_dom' {σ} (A : Carriers σ) (n : nat)
  : forall (N : n > 0) (ss : Vect n (Sort σ)) (a : forall i, A (ss i)),
    A (vhead' n N ss)
  := match n with
...
Monomorphic Definition head_dom {σ} (A : Carriers σ) {n : nat}
  (ss : Vect n.+1 (Sort σ)) (a : forall i, A (ss i))
  : A (vhead ss)
...
Monomorphic Definition tail_dom' {σ} (A : Carriers σ) (n : nat)
  : forall (ss : Vect n (Sort σ)) (a : forall i, A (ss i)) (i : Fin (pred n)),
    A (vtail' n ss i)
  := match n with
...
Monomorphic Definition cons_dom' {σ} (A : Carriers σ) {n : nat}
  : forall (i : Fin n) (ss : Vect n (Sort σ)) (N : n > 0),
    A (vhead' n N ss) -> (forall i, A (vtail' n ss i)) -> A (ss i)
  := fin_ind
      (fun n i =>
...
Definition cons_dom {σ} (A : Carriers σ)
  {n : nat} (ss : Vect n.+1 (Sort σ))
  (x : A (vhead ss)) (xs : forall i, A (vtail ss i))
  : forall i : Fin n.+1, A (ss i)
  := fun i => cons_dom' A i ss tt x xs.

(** The empty domain: *)

Definition nil_dom {σ} (A : Carriers σ) (ss : Vect 0 (Sort σ))
...
Definition FiniteOperation {σ : Signature} (A : Carriers σ)
  {n : nat} (ss : Vect n (Sort σ)) (t : Sort σ) : Type
...
Fixpoint CurriedOperation {σ} (A : Carriers σ) {n : nat}
  : (Vect n (Sort σ)) -> Sort σ -> Type
  := match n with
     | 0 => fun ss t => A t
     | n'.+1 =>
        fun ss t => A (vhead ss) -> CurriedOperation A (vtail ss) t
     end.
...
Fixpoint operation_curry {σ} (A : Carriers σ) {n : nat} 
  ...
  := match n with
     ...
     | n'.+1 =>
        fun ss t op x =>
          operation_curry A (vtail ss) t (op o cons_dom A ss x)
     end.
...
Lemma expand_cons_dom `{Funext} {σ} (A : Carriers σ)
  {n : nat} (ss : Vect n.+1 (Sort σ)) (a : forall i, A (ss i))
...
Proof.
  ...
  funext x.
  refine (_ @ IHn (vtail ss) (a x)).
  refine (ap (operation_curry A (vtail ss) t) _).
  funext a'.
  simpl.
  unfold cons_dom, cons_dom'.
  rewrite compute_fin_ind_fin_zero.
  refine (ap (operation_uncurry A (vtail ss) t (a x)) _).
  funext i'.
