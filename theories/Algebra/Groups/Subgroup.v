...
Definition subgroup_generated_in {G : Group} {X : G -> Type}
  : pred_subset X (subgroup_generated X)
  := fun _ => (tr o sgt_in).
...
Definition subgroup_generated_rec {G : Group} (X : G -> Type) (S : Subgroup G)
  (i : pred_subset X S)
  : pred_subset (subgroup_generated X) S.
Proof.
...
Definition functor_subgroup_generated {G H : Group}
  (X : G -> Type) (Y : H -> Type) (f : G $-> H) (preserves : pred_subset X (Y o f))
  : pred_subset (subgroup_generated X) (subgroup_generated Y o f).
Proof.
...
Definition subgroup_eq_functor_subgroup_generated {G H : Group}
  (X : G -> Type) (Y : H -> Type) (f : G $<~> H) (preserves : pred_eq X (Y o f))
  : pred_eq (subgroup_generated X) (subgroup_generated Y o f).
Proof.
...
Definition subgroup_eq_subgroup_generated_op {G : Group}
  (X : G -> Type) (Y : G -> Type) (preserves : pred_eq X Y)
  : pred_eq (subgroup_generated X) (subgroup_generated (G:=grp_op G) Y).
Proof.
  intro g; split.
...
Definition subgroup_generated_merely {G : Group}
  (X : G -> Type)
  : pred_eq (subgroup_generated X) (subgroup_generated (merely o X)).
Proof.
  apply pred_subset_antisymm.
  - apply functor_subgroup_generated'.
    exact (fun g => tr).
...
Definition functor_subgroup_generated_merely {G : Group}
  (X Y : G -> Type) (K : pred_subset (merely o X) (merely o Y))
  : pred_subset (subgroup_generated X) (subgroup_generated Y).
Proof.
...
Definition subgroup_eq_functor_subgroup_generated_merely {G : Group}
  (X Y : G -> Type) (K : pred_eq (merely o X) (merely o Y))
...
Definition grp_iso_subgroup_generated_merely  {G : Group}
  (X Y : G -> Type) (K : pred_eq (merely o X) (merely o Y))
...
Definition subgroup_product_ind {G : Group} (H K : Subgroup G)
  ...
  (P_H_in : forall x y, P x (subgroup_generated_in _ (inl y)))
  (P_K_in : forall x y, P x (subgroup_generated_in _ (inr y)))
...
Definition subgroup_product_incl_l {G : Group} (H K : Subgroup G)
  : pred_subset H (subgroup_product H K)
...
Definition functor_subgroup_product {G H : Group}
  ...
  (f : G $-> H) (l : pred_subset J (L o f)) (r : pred_subset K (M o f))
  : pred_subset (subgroup_product J K) (subgroup_product L M o f).
Proof.
...
Definition subgroup_eq_functor_subgroup_product {G H : Group}
  ...
  : pred_eq (subgroup_product J K) (subgroup_product L M o f).
Proof.
...
Definition subgroup_image_in {G H : Group} (f : G $-> H) (J : Subgroup G)
  : pred_subset J (subgroup_image f J o f)
...
Definition subgroup_image_rec {G H : Group}
  ...
  (g : pred_subset J (subgroup_preimage f K))
  : pred_subset (subgroup_image f J) K.
Proof.
...
Definition iff_subgroup_image_rec {G H : Group}
  (f : G $-> H) {J : Subgroup G} {K : Subgroup H}
  : pred_subset (subgroup_image f J) K
    <-> pred_subset J (subgroup_preimage f K). 
Proof.
  split.
  - intros rec x Jx.
    apply rec, tr.
