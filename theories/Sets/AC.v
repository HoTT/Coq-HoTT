...
Proof.
  ...
  intros [A HA]. eapply merely_destruct; try exact HA.
  ...
  - intros x. assert (HR' : hexists (fun y => merely (R x y * forall y', R x y' -> f y < f y' \/ f y = f y'))).
    + pose proof (HAR := ordinal_has_minimal_hsolutions A (fun a => Build_HProp (merely (exists y, f y = a /\ R x y)))).
      ...
      * eapply merely_destruct; try exact (HR x). intros [y Hy].
        apply tr. exists (f y). apply tr. exists y. by split.
      * intros [a [H1 H2]]. eapply merely_destruct; try exact H1.
        intros [y [<- Hy]]. apply tr. exists y. apply tr. split; trivial.
        intros y' Hy'. apply H2. apply tr. exists y'. split; trivial.
    + edestruct (@iota Y) as [y Hy]; try exact y. 2: split; try exact HR'. 1: exact _.
      intros y y' Hy Hy'.
      eapply merely_destruct; try exact Hy. intros [H1 H2].
  ...
  - intros x. cbn. destruct iota as [y Hy]. eapply merely_destruct; try exact Hy. by intros [].
Qed.
