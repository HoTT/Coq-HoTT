...
Section sip.
  ...
  Context `{IsUnivalentCategory X}.
  Context `{@IsStandardNotionOfStructure X P}.

  Let StrX := @category_of_structures X P.

  Definition sip_isotoid_helper (xa yb : StrX)
             (f : xa <~=~> yb)
  : xa.1 <~=~> yb.1.
  Proof.
    exists (CategoryOfStructures.f (f : morphism _ _ _)).
    exists (CategoryOfStructures.f f^-1).
    - exact (ap (@CategoryOfStructures.f _ _ _ _) (@left_inverse _ _ _ _ f)).
    - exact (ap (@CategoryOfStructures.f _ _ _ _) (@right_inverse _ _ _ _ f)).
  Defined.
  ...
  : CategoryOfStructures.f (idtoiso (category_of_structures P) p : morphism _ _ _)
    = idtoiso X p..1.
  Proof.
  ...
  Lemma structure_identity_principle_helper
        (xa yb : StrX)
        (x : xa <~=~> yb)
  : CategoryOfStructures.f
      (idtoiso (category_of_structures P) (sip_isotoid x) : morphism _ _ _)
    = CategoryOfStructures.f (x : morphism _ _ _).
  Proof.
  ...
  : IsUnivalentCategory (category_of_structures P).
  Proof.
    intros xa yb.
    ...
    - abstract (
          ...
          apply CategoryOfStructures.path_morphism;
