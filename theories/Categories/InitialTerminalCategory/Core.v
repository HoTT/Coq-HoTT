...
(** ** Terminal categories *)
(** A category is terminal if its objects and morphisms are contractible types. *)
Class IsTerminalCategory (C : Category)
      `{Contr (object C)}
      `{forall s d, Contr (morphism C s d)} := {}.

(** ** Initial categories *)
(** An initial category is one whose objects have the recursion priniciple of the empty type *)
Class IsInitialCategory (C : Category)
...
(** ** Default intitial ([0]) and terminal ([1]) categories. *)
