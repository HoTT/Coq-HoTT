...
(*   Add Ring Z : (stdlib_ring_theory Z). *)
  ...
  Global Instance integers_to_rationals_injective `{!SemiRing_Morphism (f: Z → Q)}
    : Injective f.
  Proof.
  split.
  intros x y E.
  ...
  Context `{f : Q → FracField.F Z} `{!SemiRing_Morphism f}.
  ...
  Global Instance to_frac_inverse_morphism : SemiRing_Morphism (f⁻¹) := {}.
...
Definition rationals_to_rationals_aux Z `{Integers Z}
  Q1 Q2 `{Rationals Q1} `{Rationals Q2} : Q1 → Q2
...
Section another_rationals.
  ...
  Proof.
    ...
    change ((rationals_to_frac Q1 (SRpair nat)⁻¹)
      ((rationals_to_frac Q2 (SRpair nat) ∘ f) x) = x).
    rewrite (to_frac_unique (rationals_to_frac Q2 (SRpair nat) ∘ f)
      (rationals_to_frac Q1 (SRpair nat))).
    apply (jections.bijective_applied _).
  Qed.
...
Lemma morphisms_involutive `{Rationals Q1} `{Rationals Q2} (f : Q1 → Q2)
  (g : Q2 → Q1) `{!SemiRing_Morphism f} `{!SemiRing_Morphism g} :
  ∀ x, f (g x) = x.
Proof.
  intros x.
...
Global Instance injective_nats `{Rationals Q} `{Naturals N}
  `{!SemiRing_Morphism (f : N → Q)} : Injective f.
Proof.
  split; try apply _.
  intros x y E.
  apply (injective (naturals_to_semiring N (SRpair nat))).
  apply rationals_embed_ints.
  now rewrite 2!(naturals.to_semiring_unique_alt f
    (integers_to_ring (SRpair nat) Q ∘ naturals_to_semiring N (SRpair nat))) in E.
Qed.
...
Section isomorphic_image_is_rationals.
  ...
  Instance iso_to_frac: RationalsToFrac F := λ Z _ _ _ _ _ _ _ _,
...
Section alt_Build_Rationals.
  Context `{DecField F} `{Integers Z} (F_to_fracZ : F → Frac Z)
    `{!SemiRing_Morphism F_to_fracZ} `{!Injective F_to_fracZ}.
  ...
  Program Instance alt_to_frac: RationalsToFrac F := λ B _ _ _ _ _ _ _ _ x,
    frac (integers_to_ring Z B (num (F_to_fracZ x)))
      (integers_to_ring Z B (den (F_to_fracZ x))) _.
  Next Obligation.
  ...
  Proof.
    ...
    repeat (split; try apply _); unfold equiv, Frac_equiv; simpl.
      ...
      rewrite <-(preserves_0 (f:=integers_to_ring Z B)),
        <-(preserves_1 (f:=integers_to_ring Z B)),
         <-2!preserves_mult.
      apply sm_proper.
