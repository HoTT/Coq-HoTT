...
Instance natdistance_cut_minus `{Naturals N} `{Apart N}
  `{!TrivialApart N} `{!FullPseudoSemiRingOrder Nle Nlt}
   {cm} `{!CutMinusSpec N cm} `{∀ x y, Decision (x ≤ y)} : NatDistance N.
Proof.
