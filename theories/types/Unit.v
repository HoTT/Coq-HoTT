...
Global Instance isequiv_path_unit (z z' : Unit) : IsEquiv (path_unit_uncurried z z').
...
Global Instance isequiv_unit_rect `{Funext} (A : Type) : IsEquiv (@Unit_rect A)
...
Global Instance isequiv_unit_corect `{Funext} (A : Type) : IsEquiv (@unit_corect A)
  := isequiv_adjointify _
  (fun f => tt)
  _ _.
Proof.
...
Global Instance contr_unit : Contr Unit := {
  center := tt;
...
Global Instance contr_equiv_unit (A : Type) (f : A <~> Unit) : Contr A
  := BuildContr A (f^-1 tt)
