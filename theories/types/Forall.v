...
Definition path_forall {A : Type} {B : A -> Type} (f g : forall x:A, B x)
  : (forall x:A, f x = g x) -> (f = g)
  := (@apD10 A B f g)^-1.

(** Now we show how these things compute. *)

Definition apD10_path_forall {A : Type} {B : A -> Type}
  (f g : forall x:A, B x) (h : forall x:A, f x = g x)
  : apD10 (path_forall _ _ h) = h
  := eisretr apD10 h.

Definition eta_path_forall {A : Type} {B : A -> Type}
...
Instance isequiv_path_forall {A : Type} {B : A -> Type}
  (f g : forall x:A, B x)
  : IsEquiv (path_forall f g)
  := @isequiv_inverse _ _ (@apD10 A B f g) _.

Definition equiv_path_forall {A : Type} {B : A -> Type}
  (f g : forall x:A, B x)
  : (forall x:A, f x = g x)  <~>  (f = g)
