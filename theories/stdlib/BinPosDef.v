...
Instance Ppow : Pow positive positive := fun x => iter (x *.) 1.
...
Instance: Compare positive := compare_cont EQ.
...
Instance : Eqb positive := Peqb.

Instance : Leb positive := fun x y =>
 match x ?= y with GT => false | _ => true end.

Instance : Ltb positive := fun x y =>
...
Definition gcd (a b : positive)
  := gcdn (Peano.plus (size_nat a) (size_nat b)) a b.
...
Definition ggcd (a b: positive)
  := ggcdn (Peano.plus (size_nat a) (size_nat b)) a b.
...
Definition to_nat (x:positive) : nat := iter_op Peano.plus x (S O).
