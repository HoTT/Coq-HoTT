...
(*   : Hom2FMC *)
(*     (comp (tensor2 (id w) (tensor2 f (id x))) (associator w y x)) *)
(*     (comp (associator w z x) (tensor2 (tensor2 (id w) f) (id x))) *)
...
Global Instance pentagonidentity_FMC {X} : PentagonIdentity (@tensor X)
  := @pentagon X.
...
Defined.
Locate "[]".
Global Instance leftunitor_nfmc {X : Type} : LeftUnitor (@app X) nil.
Proof.
  snapply Build_NatEquiv.
  1: reflexivity.
  apply Build_Is1Natural.
...
Global Instance rightunitor_nfmc {X : Type} : RightUnitor (@app X) nil.
Proof.
  snapply Build_NatEquiv.
  - intros l.
    induction l.
...
Global Instance triangleidentity_nfmc {X : Type} : TriangleIdentity (@app X) nil.
Proof.
  intros l m.
  induction l.
  1: reflexivity.
...
Global Instance ismonoidal_nfmc {X : Type} : IsMonoidal (NFMC X) (app (A:=X)) nil := {}.
...
Global Instance pentagonidentity_NFMC' {X : Type} : PentagonIdentity (nfmc'_tensor X).
Proof.
...
Proof.
  ...
  - (** Functoriality of tensor *)
    ...
    { (** Using naturality of the associator *)
      ...
      - napply comp2.
        1: napply wl; apply tensor2_comp.
        napply comp2.
        1: apply tensor2_comp.
        simpl.
        1: napply comp2.
    ...
    + 
      ...
      refine (_ $@ ((cat_idr _)^$ $@R _)). 
      change (fmap (nrm' X n) g) with (fmap (nfmc_fmc_incl X o nfmc'_nfmc X n) (fmap (interp X) g)).
      unfold is0functor_nrm'.
      set (p := fmap (interp X) g).
      clearbody p.
      simpl in p.
      change (?w $o ?x $== ?y $o ?z) with (Square z w x y).
      cbn -[Square "$o"].
      unfold nfmc'_nfmc.
      simpl.
      generalize (p n); clear p; intro p.
      set (x := interp X b n) in *; clearbody x; revert x p.
      apply paths_ind_r.
      napply vconcatL.
      1: apply tensor2_id.
      rapply hrefl. *)
...
Global Instance is0functor_nrm X : Is0Functor (nrm X) := _.
Global Instance is1functor_nrm X : Is1Functor (nrm X) := _.

Definition natequiv_idmap_nrm {X} : NatEquiv idmap (nrm X).
Proof.
...
Definition nrm_unique {X} {x y : FMC X} (f f' : x $-> y)
  : fmap (nrm X) f $== fmap (nrm X) f'.
Proof.
  unfold is0functor_nrm.
  unfold is0functor_nrm'.
  cbn -[is0functor_nfmc_fmc_incl is0functor_nfmc'_nfmc is0functor_interp "$=="].
  
  refine (fmap2 (nfmc_fmc_incl X) _).
  refine (fmap2 (nfmc'_nfmc X []) _).
  generalize (fmap (interp X) f); clear f; intro f.
  generalize (fmap (interp X) f'); clear f'; intro f'.
  hnf. simpl.
  simpl in f, f'.
  destruct f as [f isnat_f].
  destruct f' as [f' isnat_f'].
  simpl in *.
  hnf in isnat_f, isnat_f'.
  simpl in isnat_f, isnat_f'.
...
(*cbn.
  induction f as [
    | x y z f IHf g IHg
    | x y f IHf | | |
    | a b a' b' f IHf g IHg ] in f' |- *.
  - simpl. *)
Admitted.

Theorem monoidal_coherence {X} `{IsHSet X} {x y : FMC X} (f f' : x $-> y)
  : f $== f'.
Proof.
  pose (isnat natequiv_idmap_nrm f).
  pose (isnat natequiv_idmap_nrm f').

  refine (gpd_moveL_Vh (isnat natequiv_idmap_nrm f) $@ (_ $@L (_ $@R _))
    $@(gpd_moveL_Vh (isnat natequiv_idmap_nrm f'))^$).
