...
(** [Finite] is equivalent to a sigma type *)
Definition issig_finite (X : Type)
...
Global Instance ishprop_finite (X : Type)
  : IsHProp (Finite X).
Proof.
...
Definition finite_isequiv (X : Type) {Y : Type} (e : X -> Y) `{!IsEquiv e}
  : Finite X -> Finite Y.
Proof.
  intros ?.
...
Definition finite_equiv (X : Type) {Y : Type} (e : X <~> Y)
  : Finite X -> Finite Y
  := finite_isequiv X e.

(** If two types are equivalent then so are their proofs of [Finite] *)
Corollary finite_equiv_equiv (X Y : Type)
...
Definition fcard_isequiv {X Y : Type} (e : X -> Y) `{!IsEquiv e}
  ...
  : fcard X = fcard Y.
Proof.
  transitivity (@fcard Y (finite_isequiv X e _)).
  - reflexivity.
  - exact (ap (@fcard Y) (path_ishprop _ _)).
Defined.

(** Equivalent finite types have equal cardinalities. *)
Definition fcard_equiv {X Y : Type} (e : X <~> Y) `{Finite X} `{Finite Y}
  : fcard X = fcard Y
  := fcard_isequiv e.
...
Global Instance finite_fin (n : nat) : Finite (Fin n)
...
Global Instance finite_merely (X : Type) {fX : Finite X}
  : Finite (merely X).
Proof.
...
Global Instance finite_paths {X : Type} `{Finite X} (x y : X)
  : Finite (x = y).
Proof.
  ...
  strip_truncations.
  refine (finite_isequiv _ (ap e)^-1 _).
  apply finite_decidable_hprop; exact _.
Defined.

(** Finite sets are also closed under successors. *)
Global Instance finite_succ (X : Type) `{Finite X} : Finite (X + Unit).
Proof.
...
Definition fcard_succ (X : Type) `{Finite X}
  : fcard (X + Unit) = (fcard X).+1 := 1.

(** ** Decidability *)

(** Like canonical finite sets, finite sets have decidable equality. *)
Global Instance decidablepaths_finite `{Funext} (X : Type) `{Finite X}
  : DecidablePaths X.
Proof.
  assert (e := merely_equiv_fin X).
  strip_truncations.
...
Global Instance decidable_finite_hprop (X : Type)
  `{IsHProp X} {fX : Finite X}
  : Decidable X.
Proof.
...
Global Instance decidable_merely_finite (X : Type) {fX : Finite X}
  : Decidable (merely X) := _.

(** From this, it follows that any finite set is *merely* decidable. *)
Definition merely_decidable_finite X `{Finite X} : merely (Decidable X).
Proof.
...
Definition finite_choice {X : Type} `{Finite X} (P : X -> Type)
...
Global Instance finite_sum (X Y : Type) `{Finite X} `{Finite Y}
  : Finite (X + Y).
Proof.
  assert (e := merely_equiv_fin Y).
  strip_truncations.
  refine (finite_isequiv _ (functor_sum idmap e^-1) _).
...
Definition fcard_sum (X Y : Type) `{Finite X} `{Finite Y}
...
Global Instance finite_prod (X Y : Type) `{Finite X} `{Finite Y}
  : Finite (X * Y).
Proof.
  assert (e := merely_equiv_fin Y).
  strip_truncations.
  refine (finite_isequiv _ (functor_prod idmap e^-1) _).
  generalize (fcard Y); intros n.
  induction n as [|n IH].
...
Definition fcard_prod (X Y : Type) `{Finite X} `{Finite Y}
  : fcard (X * Y) = fcard X * fcard Y.
Proof.
  assert (e := merely_equiv_fin X).
  strip_truncations.
...
Global Instance finite_forall `{Funext} {X : Type} (Y : X -> Type)
  `{Finite X} `{forall x, Finite (Y x)}
  : Finite (forall (x : X), Y x).
Proof.
  assert (e := merely_equiv_fin X).
  strip_truncations.
  srefine (finite_equiv _ (equiv_functor_forall'
...
Global Instance finite_aut `{Funext} (X : Type) `{Finite X}
  : Finite (X <~> X).
Proof.
  assert (e := merely_equiv_fin X).
  strip_truncations.
...
Definition fcard_aut `{Funext} (X : Type) `{Finite X}
...
Global Instance finite_sigma {X : Type} (Y : X -> Type) `{Finite X}
  `{forall x, Finite (Y x)}
  : Finite (sig Y).
...
Proof.
  assert (e := merely_equiv_fin X).
  strip_truncations.
  refine (finite_equiv _ (equiv_functor_sigma (equiv_inverse e)
    (fun x (y:Y (e^-1 x)) => y)) _).
  ...
  generalize dependent (fcard X); intros n Y' ?.
  induction n as [|n IH].
  - refine (finite_isequiv Empty pr1^-1 _).
...
Definition finplus {X : Type} `{Finite X} (f : X -> nat) : nat
  := fcard { x:X & Fin (f x) }.
...
Definition finplus_const (X : Type) `{Finite X} n
...
Definition finite_image {X Y : Type} `{Finite X} `{Finite Y} (f : X -> Y)
  : Finite (himage f) := _.
...
Definition finmult `{Funext} {X : Type} `{Finite X} (f : X -> nat) : nat
...
Definition finmult_const `{Funext} (X : Type) `{Finite X} (n : nat)
...
Definition detachable_image_finite
  {X Y : Type} `{Finite X} `{Finite Y} (f : X -> Y) `{IsEmbedding f}
...
Definition detachable_finite_subset {X : Type} `{Finite X}
...
Definition leq_inj_finite `{Funext} {X Y : Type}
  {fX : Finite X} {fY : Finite Y} (f : X -> Y) (i : IsEmbedding f)
  : fcard X <= fcard Y.
Proof.
...
Section Enumeration.
  ...
  Context `{Funext} {X : Type} `{Finite X} (e : nat -> X).
