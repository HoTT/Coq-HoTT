...
Axiom funext_comp : forall {A : Type} {B : A -> Type}
                         (f : forall a: A, B a),
                    funext (fun a => eq_refl (f a)) = eq_refl f.

(* In fact [funext_comp] is sort of a consequence of [funext]; but that’s a story for another day! *)

Lemma eta_exp {A : Type} {B : A -> Type}
              (f : forall a: A, B a)
            : f = (fun a => f a).
Proof.  
  apply funext.  intro a.  apply eq_refl.
Defined.

Lemma eta_comp {A : Type} {B : A -> Type}
               (f : forall a: A, B a)
             : eta_exp (fun a => f a) = (eq_refl _).
Proof.
  apply funext_comp.  
Defined.
...
Proof.
  ...
  generalize dependent f'_good. rewrite (eta_exp f') in *.
  intro f'_good.  rewrite (eta_exp f'_good) in *.
