...
Lemma path_in_total {A} {P : A -> Type} : forall (u v : sigT P),
...
Lemma equiv_prod {A B} {C D} : A <~> B -> C <~> D -> A * C <~> B * D.
Proof.
  intros f g.
  exists (fun u => (f (fst u), g (snd u))).
  apply @hequiv_is_equiv with (g := fun v => (f^-1 (fst v), g^-1 (snd v))).
  intros (b,d).
  simpl.
...
Lemma equiv_sum {A B} {C D} : A <~> B -> C <~> D -> A + C <~> B + D.
Proof.
  intros f g.
...
Proof.
  ...
  apply @hequiv_is_equiv with
    (g := fun v => (inverse (EP f g E) (fst v), inverse (EQ f g E) (snd v))).
...
Proof.
  ...
  apply @hequiv_is_equiv with
    (g := fun v => match v with
                     | inl a => inl _ (inverse (EP f g E) a)
                     | inr b => inr _ (inverse (EQ f g E) b)
                   end).
  intros [? | ?]; apply map; apply inverse_is_section.
  intros [? | ?]; apply map; apply inverse_is_retraction.
Qed.
...
Proof.
  intros EC f g E.
  exists (fun t => (pr1 t ; EC (pr1 t) f g E (pr2 t))).
  apply @hequiv_is_equiv with
    (g := fun t => (pr1 t; inverse (EC (pr1 t) f g E) (pr2 t))).
  intros [c u].
...
Proof.
  ...
  exists (fun t =>
    let z := pr1 t in
    let u := pr2 t in
      (EC f g E z ; EP f g E z (EC f g E z) (idpath _) u)).
  apply @hequiv_is_equiv with
    (g := fun s =>
      let z := pr1 s in
        let v := pr2 s in
          ((EC f g E)^-1 z ; (EP f g E ((EC f g E)^-1 z) z (inverse_is_section _ z))^-1 v)).
  intros [c u].
  simpl in * |- *.
  set (ec := EC f g E).
  set (ep := EP f g E).
...
Proof.
  ...
  apply @hequiv_is_equiv with
    (g := fun h z => inverse (EC z f g E) (h z)).
  intro h.
  set (r := fun z h => (EC z f g E) (inverse (EC z f g E) (h z))).
  simpl in r.
Admitted.
...
Proof.
  intros EP EQ f g E.
  exists (fun h x => EQ f g E (h (inverse (EP f g E) x))).
  apply @hequiv_is_equiv with
    (g := fun h y => inverse (EQ f g E) (h (EP f g E y))).
  intro h.
Admitted.
